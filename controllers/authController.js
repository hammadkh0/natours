...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Forgot your password bitch! Submit a request with your password and confirmPassword to the url:${resetUrl}. If you didn't forget, then ignore this email and fuck off`;
...
/* 
    restrictTo is a function that takes roles as args and return a middleware function.
    Because MIDDLEWARES CANNOT take arguments from function calls. so we used a WRAPPER function.
*/
exports.restrictTo =
  (...roles) =>
  // roles = ['admin','lead-guide'] and role of person logged in => 'user' (as example).

  (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      // we have req.user because of protect middleware called before this restrictTo middleware. In protect we add user object to the response that is received here.
